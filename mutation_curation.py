...
mut.insert(loc=1, column='New_Hugo_Symbol', value=['' for i in range(mut.shape[0])])
mut['New_Hugo_Symbol'] = mut['Entrez_Gene_Id'].map(map)
...
mut = mut[~pd.isna(mut['New_Hugo_Symbol'])].reset_index(drop=True) # Dropping rows where entrnz gene id cannot map to a gene symbol
...
mut_dup = mut_dup[['New_Hugo_Symbol', 'Entrez_Gene_Id', 'Ensembl_id', 'Genome_Change']]
...
for key in gene_map.keys():
    gc = mut_dup['Genome_Change'][np.where(mut_dup['New_Hugo_Symbol']==key)[0]].values[0]
    ind = mut.loc[(mut['New_Hugo_Symbol'] == key) & (mut['Genome_Change']==gc)].index[0]
    mut.loc[ind,'New_Hugo_Symbol'] = gene_map[key]
    mut.loc[ind,'Entrez_Gene_Id'] = mut_dup['Entrez_Gene_Id'][np.where(mut_dup['New_Hugo_Symbol']==gene_map[key])[0]].values[0]
    mut.loc[ind,'Ensembl_id'] = mut_dup['Ensembl_id'][np.where(mut_dup['New_Hugo_Symbol']==gene_map[key])[0]].values[0]

mut = mut.drop_duplicates().reset_index(drop=True) # Remove duplicate rows

# All Genes
en_gs_id = mut[['Entrez_Gene_Id', 'New_Hugo_Symbol', 'Ensembl_id']]
...
#Mutation file version 2 - Binary file
def mutation_binary2(savename, protein_filter = False):
    ...
    mut_v2['DepMap_ID'] = mut_cp['DepMap_ID']
    mut_v2 = mut_v2.drop(columns = ['Variant_Classification', 'Variant_Type', 'dbSNP_RS', 'dbSNP_Val_Status'])
    mut_v2 = mut_v2.astype({'Chromosome': 'str'})
    mut_nodup = mut_v2.iloc[:,range(11)].drop_duplicates()
    mut_nodup_gc = mut_nodup.drop_duplicates(subset = ['Genome_Change'])
...
if __name__ == "__main__":
    #Unfiltered
    print('Processing Mutation_AID_binary2')
    mutation_binary2('Mutation_AID_binary2', False)
