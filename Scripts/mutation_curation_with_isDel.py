# Included isDeleterious and Annotation_Transcripts columns. Also does some processing on the columns to unify metadata
# for rows having different Annotation_Transcript for the same Genome_Change.
...
# Checking if rows having the same value of 'Genome Change' has the same value of 'Annotation_Transcript'
mut_dup = mut[mut.duplicated(subset = ['Genome_Change'], keep=False)].reset_index(drop=True) #mutations with duplicate genome change
g_diff = []
unique_gen_chnge = pd.unique(mut_dup['Genome_Change'])
for g in unique_gen_chnge:
    ind = np.where(mut_dup['Genome_Change']==g)[0]
    if len(pd.unique(mut_dup['Annotation_Transcript'][ind].values)) == 1:
        continue
    else:
        g_diff.append(g)

#get all mutations corresponding to g_diff genome changes
g_diff_ind = mut[mut['Genome_Change'].isin(g_diff)].index.to_list()
mut_g_diff = mut[mut['Genome_Change'].isin(g_diff)].sort_values(by=['Genome_Change']).reset_index(drop=True) # Has 4523 rows
#Remove rows from the mut file corresponding to those in mut_g_diff
mut_drop = mut.drop(g_diff_ind).reset_index(drop=True)

'''For the mutations in these 4523 rows in mut_g_diff, we do the following. 
For each unique mutation (indicated by a unique Genome_Change), we shall use the same information for the above columns across the rows of this mutation. 
If one row of the mutation has a “isDeleterious” status of True, use the meta information of that row to fill all other rows. 
If none of the rows has a “isDeleterious” status of True, just randomly select a row for filling the information of other rows.
'''
gene_change = pd.unique(mut_g_diff['Genome_Change'])

for g in gene_change:
    mut_sel = pd.DataFrame(columns = mut_g_diff.columns)
    ind_g = np.where(mut_g_diff['Genome_Change']==g)[0] # all rows with the same genome change
    ind = mut_g_diff.index[(mut_g_diff['Genome_Change']==g) & (mut_g_diff['isDeleterious']==True)].to_list() # checking how many of the rows has deleterious mutations
    if len(ind) == 0:
        #Replace meta information for rows with genome change with any of the randomly selected rows
        sel_row = mut_g_diff.iloc[[ind_g[0]]]
    else: 
        sel_row = mut_g_diff.iloc[[ind[0]]]
    gs = mut_g_diff.iloc[ind_g]['Gene_symbol']
    ent = mut_g_diff.iloc[ind_g]['Entrez_Gene_Id']
    en = mut_g_diff.iloc[ind_g]['Ensembl_id']
    dp = mut_g_diff.iloc[ind_g]['DepMap_ID']
    for i in range(len(ind_g)):
        mut_sel = pd.concat([mut_sel, sel_row], axis=0)
    mut_sel['Gene_symbol'] = gs.to_list()
    mut_sel['Entrez_Gene_Id'] = ent.to_list()
    mut_sel['Ensembl_id'] = en.to_list()
    mut_sel['DepMap_ID'] = dp.to_list()
    mut_drop = pd.concat([mut_drop, mut_sel], axis=0)

mut = mut_drop.reset_index(drop=True)

def mutation_long(savename):
    mut_long = mut.copy()
    mut_long.insert(loc=3, column='Source', value='CCLE')
    col1 = mut_long.pop('isDeleterious')
    mut_long = mut_long.drop(columns=['dbSNP_RS', 'dbSNP_Val_Status', 'cDNA_Change', 'Codon_Change'])
    mut_long = mut_long.iloc[:,:-13] # Drop last 13 columns
    col = mut_long.pop('DepMap_ID')
    mut_long.insert(loc =3, column = col.name, value= col)
    mut_long.insert(loc =15, column = col1.name, value= col1)
    # Add RRID ID
...
def mutation_binary2(savename, protein_filter = False):
    ...
    mut_v2['isDeleterious'] = mut_cp['isDeleterious']
    mut_v2['Annotation_Transcript'] = mut_cp['Annotation_Transcript']
    ...
    mut_nodup = mut_v2.iloc[:,range(16)].drop_duplicates() # Drop duplicates 
    ...
    mutbin_file = mut_nodup_gc.iloc[:,range(16)].copy()
    for i in range(len(map)):
...
if __name__ == "__main__":
    #Unfiltered
    print('Processing Mutation_RRID_binary')
    mutation_binary2('Mutation_RRID_binary_isDel', False)
    print('Processing Mutation_RRID_count')
    mutation_count('Mutation_RRID_count_isDel', False)
    print('Processing Mutation_RRID_long_format')
    mutation_long('Mutation_RRID_long_format_isDel')
